...
RSpec.describe 'when opening the shopping details page', type: :feature do
  before(:each) do
    ...
    @shopping1 = Shopping.create(name: 'Apples', amount: 6, author: @user, groups: [@group1, @group2])
    @shopping2 = Shopping.create(name: 'Bananas', amount: 4, author: @user, groups: [@group1])
    visit(group_shopping_path(@group1, @shopping1))
  end
  ...
  context 'shows the right content' do
    ...
    it 'shows edit button' do
      expect(page).to have_link('edit', href: edit_group_shopping_path(@group1, @shopping1))
    end
  end
  ...
  context 'when clicking the delete button' do
    it 'redirects to that group\'s transactions page' do
      click_button('delete')
      expect(page).to have_current_path(group_shoppings_url(@group1))
    end
  ...
  context 'when clicking on edit button' do
    it 'redirects to a form that adds a new transaction' do
      click_link('edit')
      expect(page).to have_current_path(edit_group_shopping_path(@group1, @shopping1))
    end
  end
end