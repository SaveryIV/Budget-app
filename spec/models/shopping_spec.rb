...
RSpec.describe Shopping, type: :model do
  ...
  context 'user' do
    it 'is valid when attribute exists' do
      expect(Shopping.create(valid_attributes)).to be_valid
    end

    it 'is not valid when attribute is blank' do
      expect(Shopping.create(no_author)).to_not be_valid
    end
  end
  ...
  context 'name' do
    ...
    it 'is not valid when attribute is blank' do
      expect(Shopping.create(no_name)).to_not be_valid
    end

    it 'is not valid when attribute length is more than 36' do
      expect(Shopping.create(name_too_long)).to_not be_valid
    end
  end

  context 'amount' do
    it 'is valid when attribute is integer' do
      expect(Shopping.create(amount_integer)).to be_valid
    end

    it 'is not valid when attribute is blank' do
      expect(Shopping.create(no_amount)).to_not be_valid
    end

    it 'is valid when attribute is float' do
      expect(Shopping.create(amount_float)).to be_valid
    end

    it 'is not valid when attribute is negative' do
      expect(Shopping.create(amount_negative)).to_not be_valid
    end

    it 'is not valid when attribute is string' do
      expect(Shopping.create(amount_string)).to_not be_valid
    end
  end
end